...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, onInsert }) => {
  ...
  // 6) State to track active tab
  const [activeTab, setActiveTab] = React.useState<'METAR' | 'ROUTE'>('METAR');

  // 7) Don't render anything if modal is closed
  if (!isOpen) return null;

  // 8) Fetch METAR given ICAO or location string
  ...
  // 9) Fetch IFR route polyline between two ICAO codes
    ...
    <div className='fixed inset-0 flex items-center justify-center z-50'>
      ...
      '>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* 13) Conditional content based on active tab */}
          {activeTab === 'METAR' && (
            <div className='flex flex-col space-y-4'>
              <h3 className='uppercase tracking-wide text-lg'>Get METAR</h3>
              <div
                className={
                  `grid items-center gap-4 grid-cols-[7fr_1fr_1fr] max-[660px]:grid-cols-2`
                }
              >
                <input
                  type='text'
                  ...
                  placeholder-gray-500 max-[660px]:col-span-2`
                  }
                />
                <Button
                  variant='default'
          ...
          {activeTab === 'ROUTE' && (
            <div className='flex flex-col space-y-4'>
              ...
              <div className='grid items-center grid-cols-[7fr_2fr] gap-4'>
                ...
                <Button className='w-full self-end' onClick={() => fetchRoute(fromICAO, toICAO)}>
                  GET
                </Button>
              </div>
              <div className='mt-4 flex flex-col space-y-2'>
